<x-layouts::app :title="__('Student Dashboard')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <flux:heading size="xl" class="font-semibold text-zinc-900 dark:text-white">
                            {{ auth()->user()->name . "'s dashboard" ?? 'Student' }}</flux:heading>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <flux:badge color="indigo" size="sm">{{ __('Student Class') }}</flux:badge>
            ...
            <div
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <flux:badge color="indigo" size="sm" class="font-medium">{{ 'Overview' }}
                        </flux:badge>
                    ...
                    <div class="rounded-3xl bg-zinc-50 p-4 text-sm text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                        <div class="flex items-center justify-between">
                            <span>{{ __('Module Completed') }}</span>
                            <span class="font-semibold">{{ $materials ?? '13' }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span>{{ __('Uncompleted Quiz') }}</span>
                            <span class="font-semibold">{{ $materials ?? '9' }}</span>
                        </div>
                    </div>
                </div>
                <div
                    class="mt-6 flex items-center justify-between border-t border-neutral-100 pt-4 dark:border-neutral-800">
                    <flux:text size="xs" class="text-zinc-500">{{ __('Last updated') }}
                        {{ now()->format('M d') }}
                    </flux:text>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            {{-- Left Column: Latest Learning Materials (Card Terpisah) --}}
            <div class="space-y-6 lg:col-span-2">
                <div class="rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-700 dark:bg-zinc-900">
                    <div
                        ...
                        <div>
                            <flux:heading size="md" class="font-bold text-zinc-900 dark:text-white">
                                {{ __('Latest learning materials') }}
                            </flux:heading>
                            <flux:subheading size="xs" class="mt-0.5 text-zinc-500">
                                {{ __('Newest modules published for your class') }}
                            </flux:subheading>
                        </div>
                    ...
                    <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        @foreach ($recentMaterials as $item)
                            <div
                                class="flex flex-col justify-between rounded-xl border border-neutral-200/80 bg-neutral-50/50 p-4 transition-all hover:border-neutral-300 dark:border-neutral-800 dark:bg-zinc-800/40 dark:hover:border-neutral-700">
                                <div class="space-y-3">
                                    <div class="flex items-center justify-between gap-2">
                                        <flux:badge color="zinc" size="sm" class="truncate">
                                            {{ $item['subject'] }}</flux:badge>
                                        <flux:text size="xs" class="shrink-0 text-zinc-400">{{ $item['date'] }}
                                        </flux:text>
                                    </div>
                                    <flux:heading size="sm"
                                        class="font-semibold leading-snug text-zinc-900 dark:text-white line-clamp-2">
                                        {{ $item['title'] }}
                                    </flux:heading>
                                    <flux:text size="xs" class="text-zinc-500 dark:text-zinc-400">
                                        {{ __('Instructor:') }} {{ $item['author'] }}
                                    </flux:text>
                                </div>
                                <div
                                    class="mt-4 flex items-center justify-between border-t border-neutral-200/60 pt-3 dark:border-neutral-700/60">
                                    ...
                                    <flux:button :href="route('student.material')" wire:navigate variant="subtle"
                                        size="xs" icon-trailing="arrow-right">
                                        {{ __('Read') }}
                                    </flux:button>
                                </div>
                            </div>
                ...
                {{-- Quiz Evaluation History Card (Terpisah dari Materials) --}}
                <div
                    ...
                    <div
                        ...
                        <div>
                            <flux:heading size="md" class="font-bold text-zinc-900 dark:text-white">
                                {{ __('Quiz evaluation history') }}
                            </flux:heading>
                            <flux:subheading size="xs" class="mt-0.5 text-zinc-500">
                                {{ __('Self-assessment quiz scores and progress') }}
                            </flux:subheading>
                        </div>
                    ...
                    <div class="mt-4 space-y-3">
                        ...
                        @foreach ($quizHistory as $quiz)
                            <div
                                class="flex items-center justify-between rounded-xl border border-neutral-200/70 bg-neutral-50/50 p-4 dark:border-neutral-800 dark:bg-zinc-800/40">
                                <div class="space-y-1">
            ...
            {{-- Right Column: Learning Progress Card (Card Terpisah) --}}
            <div class="lg:col-span-1">
                <div
                    ...
                    <div class="border-b border-neutral-100 pb-4 dark:border-neutral-800">
                        ...
                        <flux:subheading size="xs" class="mt-0.5 text-zinc-500">
                            {{ __('Current module completion rates') }}
                        </flux:subheading>
                    </div>
